...
```bash
$ aixdeb '[OP_2 OP_DROP sig1 pub1 OP_DUP OP_HASH160 hash160(pub1) OP_EQUALVERIFY OP_CHECKSIG]' --pretend-valid=sig1:pub1
aixdeb 0.2.16 -- type `./aixdeb -h` for start up options
valid script
...
These closely resemble the aixdeb tf command. For more information, type `tf -h` from inside aixdeb.
